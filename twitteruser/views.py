...
def user_profile(request, user_id):
  profile = TwitterUserModel.objects.get(id=user_id)
...
def following_view(request, following_id):
    ...
    follow = TwitterUserModel.objects.filter(id=following_id).first()
    current_user.following.add(follow)
    return HttpResponseRedirect(request.get('HTTP_REFERER', '/'))


@login_required
def unfollow_view(request, unfollow_id):
    current_user = request.user
    follow = TwitterUserModel.objects.filter(id=unfollow_id).first()
